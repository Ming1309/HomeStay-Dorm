...
- Request tạo lịch không còn nhận `MaCN`; `TaoLichHen` đọc lại `NhanVien.MaCN` và snapshot vào `LichHen`.
- Danh sách chứng từ, tra cứu và cập nhật lịch đều lọc `MaCN` tại SQL; lịch khác chi nhánh được xử lý như không tồn tại.
- Unique filtered index chặn hai lịch đang hoạt động trùng nhân viên, ngày và giờ.
