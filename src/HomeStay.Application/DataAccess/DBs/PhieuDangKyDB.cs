...
public static class PhieuDangKyDB
{
    ...
    public static async Task<bool> KiemTraConHopLe(string maPDK, string maCN)
    {
        const string sql = "SELECT COUNT(1) FROM PhieuDangKy WHERE MaPDK=@MaPDK AND MaCN=@MaCN AND TrangThai=N'DangXuLy'";
        return await PhienDuLieu.Session.Connection.ExecuteScalarAsync<int>(
            sql, new { MaPDK = maPDK, MaCN = maCN }, PhienDuLieu.Session.Transaction) > 0;
    }
    ...
    public static async Task<PhieuDangKy?> LayTheoMa(string maPDK, string maCN)
    {
        const string sql = """
            SELECT pdk.MaPDK,pdk.KhuVuc,pdk.SoLuongNguoi,pdk.LoaiDichVu,pdk.MucGia,
                   pdk.ThoiGianDuKienVao,pdk.ThoiHanThue,pdk.YeuCauKhac,pdk.TrangThai,pdk.MaNV,pdk.MaCN,pdk.MaKH,
            ...
            WHERE pdk.MaPDK=@MaPDK AND pdk.MaCN=@MaCN
            """;
        var rows = await PhienDuLieu.Session.Connection.QueryAsync<PhieuDangKy, KhachHang, PhieuDangKy>(
            ...
            new { MaPDK = maPDK, MaCN = maCN }, PhienDuLieu.Session.Transaction, splitOn: "MaKH");
        return rows.SingleOrDefault();
    }

    public static async Task<IReadOnlyList<PhieuDangKy>> TimKiem(string maCN, string? sdt, string? soGiayTo, string? email, string? hoTen = null, string? maPDK = null)
    {
        ...
        const string sql = """
            ...
            FROM PhieuDangKy pdk
            LEFT JOIN KhachHang kh ON pdk.MaKH=kh.MaKH
            WHERE pdk.MaCN=@MaCN AND ((@SDT IS NOT NULL AND kh.SDT LIKE '%' + @SDT + '%')
