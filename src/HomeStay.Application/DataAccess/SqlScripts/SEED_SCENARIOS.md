...
- `PC0001` - `PC0008` phủ toàn bộ trạng thái phiếu cọc, cả `OGhep` và `NguyenCan`.
...
`03_Auth.sql` tạo các tài khoản phát triển `admin`, `sale`, `ketoan`, `quanly` để kiểm thử phân quyền. Các màn hình frontend vẫn đang dùng mock TypeScript riêng; seed SQL chỉ tác động đến database.
