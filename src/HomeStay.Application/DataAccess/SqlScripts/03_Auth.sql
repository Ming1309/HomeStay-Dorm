-- Cấu hình xác thực và tài khoản mẫu.
-- Chạy sau 01_InitTables.sql và 02_Seeds.sql.
USE HomeStay;
GO
...
BEGIN
    ...
    WHERE c.object_id = OBJECT_ID(N'TaiKhoan')
      AND c.name = N'BatBuocDoiMatKhau';
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.columns
    WHERE object_id = OBJECT_ID(N'NhanVien')
      AND name = N'VaiTro'
)
...
IF NOT EXISTS (SELECT 1 FROM NhanVien WHERE MaNV = 'NV_ADMIN')
    ...
    VALUES ('NV_ADMIN', N'Quản trị hệ thống', NULL, N'QuanTri', 'CN01');
GO

-- Tài khoản phát triển dùng chung cho kiểm thử từng vai trò.
