...
public sealed record TepMinhChungThuHoi(string TenTep, long KichThuoc, Stream NoiDung);

public sealed record NoiDungMinhChungThuHoi(byte[] DuLieu, string ContentType);

public interface IMinhChungThuHoiStorage
{
    Task<string> Luu(TepMinhChungThuHoi tep, CancellationToken cancellationToken = default);
    Task Xoa(string duongDan, CancellationToken cancellationToken = default);
    Task<NoiDungMinhChungThuHoi?> Doc(string tenTep, CancellationToken cancellationToken = default);
}

public sealed class MinhChungThuHoiFileStorage(string thuMucGoc) : IMinhChungThuHoiStorage
{
    public const long DungLuongToiDa = 5 * 1024 * 1024;
    public const string ThongBaoTepKhongHopLe =
        "Định dạng tệp không hỗ trợ hoặc dung lượng quá lớn. Vui lòng tải lên file ảnh dưới 5MB.";
    ...
    public async Task<string> Luu(TepMinhChungThuHoi tep, CancellationToken cancellationToken = default)
    {
        ...
        return $"/api/asset-recovery/proofs/{tenTep}";
    }
    ...
    public async Task<NoiDungMinhChungThuHoi?> Doc(string tenTep, CancellationToken cancellationToken = default)
    {
        ...
        return new NoiDungMinhChungThuHoi(await File.ReadAllBytesAsync(duongDanVatLy, cancellationToken), contentType);
    }
